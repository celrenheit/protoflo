...
class Port (EventEmitter):
    ...
    def __init__(self, datatype="all", required=False, **options):
        ...
        if datatype not in validTypes:
            raise Error(
                "Invalid port datatype {:s} specified".format(datatype))

        if "type" in options and "/" not in options["type"]:
            raise Error("Invalid port type {:s} specified. Should be URL or MIME type.".format(
                options["type"]))
    ...
    def detach(self, socket):
        try:
            index = next(k for k, v in self.sockets.items() if v == socket)
    ...
    def connected(self, index=None):
        if self.addressable:
            ...
            try:
                ...
                raise IndexError(
                    "{:s}: Socket {:d} not available".format(self.id, index))

        return any(s.connected for s in self.sockets.values())
...
class InPort (Port):
    ...
    def __init__(self, process=None, **options):
        ...
        if process is None or isinstance(process, collections.Callable):
    ...
    def attachSocket(self, socket, index=None):
        ...
        for e in ("connect", "disconnect"):
            socket.on(e, functools.partial(
                handle, e, socket=socket, index=index))
    ...
    def handleSocketEvent(self, event, data, index=None, socket=None):
        ...
        if self.process is not None:
            if self.addressable:
                self.process(event, index=index,
                             nodeInstance=self.nodeInstance, data=data)
            else:
        ...
        if self.addressable:
            self.emit(event, index=index,
                      nodeInstance=self.nodeInstance, **data)
        else:
    ...
    def sendDefault(self):
        ...
        for index, socket in self.sockets.items():
